...
class PreTrainedTokenizer(object):
    ...
    def add_tokens(self, new_tokens, offset=None):
        """
        ...
        Args:
            ...
            offset: int. Where the new tokens are inserted. Use `1` to add them in front. Default is at the end: `len(self)`
        Returns:
        ...
        for token in new_tokens:
            ...
            if (
                ...
                logger.info("Adding %s to the vocabulary at id %d" % (token, current_id))
                current_id += 1

        added_tok_encoder = dict((tok, offset + i) for i, tok in enumerate(to_add_tokens))
        added_tok_decoder = {v:k for k, v in added_tok_encoder.items()}
